...
if __name__ == '__main__':
    ...
    f3 = open(parameters.test_file_path,
              encoding='utf-8').readlines()

    testLabelList = []
    for item_f3 in f3:
        if len(item_f3) > 0:
            testLabelList.append(labelIndexDic[item_f3.strip()])
    ...
    labelEmbed_teaching_test = []
    labelEmbed_predict_test = []

    for item_l in testLabelList:
        tmp = [1]
        ...
        labelEmbed_teaching_test.append(tmp)
        ...
        tmp.append(2)
        labelEmbed_predict_test.append(tmp)

    labelEmbed_teachingArray_test = np.array(labelEmbed_teaching_test)
    labelEmbed_predictArray_test = np.array(labelEmbed_predict_test)
    ...
    for item_f3 in tqdm(f3):
        ...
        testData.append(imgInfo)
        testPosition.append(positionVec)

    np.save(root_path + '/data/preprocess_data/testData_160', np.array(testData))
    np.save(root_path + '/data/preprocess_data/testPosition_160', np.array(testPosition))
